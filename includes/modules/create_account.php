...
if (isset($_POST['action']) && ($_POST['action'] == 'process')) {
  ...
  if (ACCOUNT_COMPANY == 'true')
  {
   $company = zen_db_prepare_input($_POST['company']);
	 $intracom = zen_db_prepare_input($_POST['intracom']);
   }
...
//echo strlen($intracom); exit;
  ...
  // FV gestion des INTRACOM
  /*
if (  ($lg_code==2) &&  strlen($company) && ($country==21) && (strlen($intracom)==0)
     ||  ($lg_code==4) &&  strlen($company)  && (strlen($intracom)==0)  )  
{
  $error = true;
  $messageStack->add('create_account', 'intracom');
}
else 
*/
if ( (strlen($company)>0) && ($country==21) && (strlen($intracom)!=11) )
{
  $error = true;
  $msg='Ne pas introduire de caractères de séparation. <br>
Le numéro intracommunautaire doit respecter le format suivant: "BE" + "9 caractères"<br>
Exemple : BEN12345678<br>';

  $messageStack->add('create_account', $msg);
}
else if ( (strlen($company)>0) && ($country==81) && (strlen($intracom)!=11) )
{
  $error = true;
  $msg='
Die Mehrwertsteuernummer soll das folgende Format respektieren: DE + 9 Zahlen <br>
Zum Beispiel : DE123456789<br>';
  $messageStack->add('create_account', $msg);
}
else if ( (strlen($company)>0) && ($country==14) && (strlen($intracom)!=11) )
{
  $error = true;
  $msg='
Die Mehrwertsteuernummer soll das folgende Format respektieren: AUT + 8 Zahlen <br>
Zum Beispiel : AUT12345678<br>';
  $messageStack->add('create_account', $msg);
}
else if ( (strlen($company)>0) && ($country==204) && (strlen($intracom)!=8) )
{
  $error = true;
  $msg='
Die Mehrwertsteuernummer soll das folgende Format respektieren: CH + 6 Zahlen <br>
Zum Beispiel : CH123456<br>';
  $messageStack->add('create_account', $msg);
}
else if ( (strlen($company)>0) && ($country==204) && (strlen($intracom)!=8) )
{
...
else if ( (strlen($company)>0) &&  ( ($country==195) || ($country==196) ) && (strlen($intracom)!=11) )
{
  $error = true;
  $msg=strlen($intracom).'
Por favor, no introduzca ningún carácter de separación.
El n° de IVA Intracomunitario debe tener "ES" + "9 caracteres"<br>
Ejemplo : ESN12345678';
  $messageStack->add('create_account', $msg);
}
else if  ( (strlen($intracom)==0) &&  ! ( ($country==73) || ($country==204) ) )
{
  $error = true;
  $msg='Missing VAT Number';
  $messageStack->add('create_account', $msg);  
}
  ...
  if ($error == true) {
    ...
    if (ACCOUNT_COMPANY == 'true') $sql_data_array['entry_tva_intracom'] = $intracom;
    ...
    if (SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO_STATUS == '1' and SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO !='') {
	  ...
	  if ( $_SESSION['customer_country_id'] == 73 )
	     $account_copy  = SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO;
	  else
	     $account_copy = "ingoriess@easylamps.eu";
      ...
      zen_mail('',$account_copy , SEND_EXTRA_CREATE_ACCOUNT_EMAILS_TO_SUBJECT . ' ' . EMAIL_SUBJECT,

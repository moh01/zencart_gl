...
//        $lc_text = '<a href="' . zen_href_link(FILENAME_DEFAULT, 'manufacturers_id=' . $listing->fields['manufacturers_id']) . '">' . $listing->fields['manufacturers_name'] . '</a>';
        $lc_text = $listing->fields['manufacturers_name'].'|'.zen_get_products_description($listing->fields['products_id'], $_SESSION['languages_id']).'|'.$listing->fields['products_id'] ;
...
//echo  '.'.$lc_price.'.';exit;
        ...
        if (  ($_SESSION['customer_id']) && ( $lc_price != '') )
          $lc_text = '<a href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing->fields['products_id']) . '"><img border=0  VSPACE=8 HSPACE=8  src="includes/templates/template_default/buttons/'.$_SESSION['language'].'/'. BUTTON_IMAGE_BUY_NOW.'" alt='.BUTTON_BUY_NOW_ALT.' class="listingBuyNowButton"' . '></a>';
		else
		  $lc_text = "&nbsp;";
        break;
        case 'PRODUCT_LIST_WEIGHT':

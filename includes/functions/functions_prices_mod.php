...
//  	  $product_price = $product->fields['products_price'];
   ...
   if ($specials->RecordCount() > 0 && $specials->fields['specials_new_products_price'] < $product_price) {
...
if ($_SESSION['customer_id']) {
	...
	$customer_check = $db->Execute("select * from " . TABLE_CUSTOMERS . " where customers_id = '$customers_id'");
		...
		$options_values_price = $optionprice[$i];
		
		if ($options_values_price=='0' || $options_values_price==''){
			$options_values_price = $optionprice[0];
		}
...
//           $the_base_price += $product_att_query->fields['options_values_price']; //REMOVED FOR WHOLESALE PRICING MOD
          }
...
//****************************************************************
//***** END Generate Net Retail Price for Displaying with Trade Price
//****************************************************************

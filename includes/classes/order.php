  ...
  function  create_add_products($zf_insert_id, $zf_mode = false) {
    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
      ...
      $sql = "select m.manufacturers_name,
              ...
              and   p.products_id=" . $this->products[$i]['id'];
...
//                              'products_name' => $this->products[$i]['name'],
                              'products_model' => $this->products[$i]['name'], 
                              'products_name' => $prdd, 							  							  
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
    ...
    if ($this->info['payment_method']=='Vorkasse/Überweisung')
       $html_msg['EMAIL_DETAILS_FOLLOW'] .= '<br><br><font color=red>"Der Versand Ihrer Bestellung erfolgt erst nach Eingang Ihrer Bezahlung bei unserer Bank."</font><br><br>';
    ...
    if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
	  if ( $_SESSION['customer_country_id'] == 73 )
	     $cc_cust = "info@easylamps.eu";
	  else
	     $cc_cust = "ingoriess@easylamps.eu";
	  
      $extra_info=email_collect_extra_info('','', $this->customer['firstname'] . ' ' . $this->customer['lastname'], $this->customer['email_address'], $this->customer['telephone']);
      $html_msg['EXTRA_INFO'] = $extra_info['HTML'];
      zen_mail('', $cc_cust , SEND_EXTRA_NEW_ORDERS_EMAILS_TO_SUBJECT . ' ' . EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id,

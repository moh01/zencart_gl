...
class messageStack extends tableBox {
  ...
  function add($class, $message, $type = 'error') {
    ...
    if (strlen($message) > 0) {
      if ($message == 'intracom') {
        if ( $lg_code == 2 )
           $msg = 'Veuillez indiquer votre numéro de TVA Intracommunaitaire ("BE" + 3 groupes de 3 chiffres sans les séparateurs)';
        else if ( $lg_code == 4  )
           $msg = 'Bitte geben Sie ihre UST-Id.-Nummer an'.$_POST['country'];
        else
           $msg = $lg_code;

        $this->messages[] = array('params' => 'class="messageStackSuccess larger"', 'class' => $class, 'text' => zen_image($template->get_template_dir(ICON_IMAGE_SUCCESS, DIR_WS_TEMPLATE, $current_page_base,'images/icons'). '/' . ICON_IMAGE_SUCCESS, ICON_SUCCESS_ALT) . '  ' . $msg );
      } elseif ($type == 'error') {

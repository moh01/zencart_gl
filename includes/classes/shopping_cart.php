 ...
 */
   ...
   function get_el_price ( $customer, $products_id )
  {
    global $db;
       
        //  FV do not select dia
		$sql =  "select price
			 from   el_price, customers, categories as cat,  
			 ...
			 where  cat.categories_id = catd.categories_id
               ...
               and    prd.products_id = " .  $products_id . "
               and    prdd.products_id = prd.products_id
               and    prd.master_categories_id = cat.categories_id 
               and  (  ( price_list_id =  main_price_list_id
						 and    customers.customers_id = " . $customer  . " )
					  or 
					   (  el_price.customer_id = " . $customer . " 
						  and  el_price.date_created = CURRENT_DATE () )
					 )     
			 and    ctr_code = cstrd.categories_name
			 and    lamp_code = prdd.products_name
			 order  by  el_price.customer_id desc ";                             
//  echo $sql;exit;                                  
      //             $prix1 = exec_select($sql);

      $prix1_rc = $db->Execute( $sql );
      $response = $prix1_rc->fields['price'];
	  // echo $response;exit;
      return $response;	  
  }
    ...
    while (list($products_id, ) = each($this->contents)) {
      ...
      if ($product = $db->Execute($product_query)) {
        ...
        $customer = $_SESSION['customer_id'];
     	...
     	$products_price = get_el_price ( $customer , $products_id  );
...
//	  return -1;
      $attributes_price = $the_base_price;
  }
  else
  {
     	  $attributes_price =  -2;
  ...
  function get_products($check_for_valid_cart = false) {
    ...
    while (list($products_id, ) = each($this->contents)) {
      $products_query = "select p.products_id, p.master_categories_id, p.products_status, pd.products_name, p.products_model, p.products_image,
                                  ...
                                  p.products_discount_type, p.products_discount_type_from, pd.products_description
...
// validate cart contents for checkout
        ...
        if ($new_qty == (int)$new_qty) {
          $new_qty = (int)$new_qty;
               ...
               AND prdd.products_id = '" .  $products_id . "'"  ;       
	   ...
	   include_once('el_admin/el_functions.php');
   	   ...
   	   $desc = get_product_desc ($manu_code,$ref_vp, $_SESSION['languages_id'] , $constructeur );

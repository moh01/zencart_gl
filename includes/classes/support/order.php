...
*/
  ...
  function  create_add_products($zf_insert_id, $zf_mode = false) {
    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
      ...
      // fv
      $sql = "select pd.products_description from products_description pd   where pd.products_id=" . $this->products[$i]['id'];
      ...
      '<td class="product-details" valign="top">' . $this->products[$i]['name']. $products_desc . ($this->products[$i]['model'] != '' ? ' (' . $this->products[$i]['model'] . ') ' : '') .
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
    ...
    $html_msg['PRODUCTS_DETAIL']='<table class="product-details" border="0" width="100%" cellspacing="0" cellpadding="2">' .str_replace('&euro;','Euros ', $this->products_ordered_html). '</table>';
    ...
    for ($i=0, $n=sizeof($order_totals); $i<$n; $i++) {
	  ...
	  if (  $order_totals[$i]['sort_order'] == 250 )
	  {
         $html_ot .= '<td class="order-totals-text" align="right" width="100%">' . $order_totals[$i]['title'] . '</td><td class="order-totals-num" align="right" nowrap="nowrap"><hr><b><big>' .($order_totals[$i]['text']) .'</big></b><hr></td></tr><tr>' ;
	  }	 
    ...
    $html_ot = str_replace('&euro;','Euros ',  $html_ot  );
	...
	// fv  <div class="holder">
//	$html_msg = "ID Client " . $this->customer['id'] . "<br>" . $this->customer['telephone'] . "  " . $this->customer['email']. "  " . $html_msg;
    $prds = $db->Execute('select * from orders_products where orders_id = '. $zf_insert_id );
    while (!$prds->EOF) 
	{
       $prod_str .= $prds->fields['products_quantity'] . " x " . $prds->fields['products_name'] . '     ' . $prds->fields['products_model']. '    '. $prds->fields['final_price']. '<br>';
	   $prds->MoveNext();
	}
				 
				 
    $html_msg['EMAIL_THANKS_FOR_SHOPPING'] = "TELEPHON:".$this->customer['telephone'] . "\n" .
                                  "<br>EMAIL:".$this->customer['email_address'] . "\n  <br>" . $prod_str ."<br><br>".  EMAIL_THANKS_FOR_SHOPPING;

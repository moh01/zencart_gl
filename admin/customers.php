  ...
  if (zen_not_null($action)) {
    switch ($action) {
      ...
      case 'update':
        ...
        $main_price_list_id = (int)zen_db_prepare_input($_POST['main_price_list_id']);
...
//$entry_email_address_check_error = true;
      ...
      if ($error == false) {
        ...
        $sql_data_array = array('customers_firstname' => $customers_firstname,
                                ...
                                'main_price_list_id' => $main_price_list_id,
      ...
      default:
        $customers = $db->Execute("select c.customers_id, c.customers_gender, c.customers_firstname,
                                          ...
                                          c.customers_email_format, c.customers_group_pricing,c.main_price_list_id,
...
?>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading">Fournisseurs</td>
...
<?php
    if ($error == true) {
      ...
      echo zen_draw_input_field('entry_tva_intracom', $cInfo->entry_tva_intracom, zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_tva_intracom', 50));	  
    }
?></td>
          </tr>
        </table></td>
...
?></td>
          ...
          <tr>
            <td class="main">Liste de Prix EasyLamps</td>
            <td class="main">
<?php
  if ($processed == true) {
    if ($cInfo->main_price_list_id) {
      $price_list_query = $db->Execute("select price_list_id, name from el_price_list ");
      echo $price_list_query->fields['name'];
    } else {
      echo ENTRY_NONE;
    }
    echo zen_draw_hidden_field('main_price_list_id', $cInfo->main_price_list_id);
  } else {
    $price_list_query = $db->execute("select price_list_id, name from el_price_list");
    $price_list[] = array('id'=>0, 'text'=>TEXT_NONE);
    while (!$price_list_query->EOF) {
      $price_list[] = array('id'=>$price_list_query->fields['price_list_id'], 'text'=>$price_list_query->fields['name']);
      $price_list_query ->MoveNext();
    }
    echo zen_draw_pull_down_menu('main_price_list_id', $price_list, $cInfo->main_price_list_id);
  }
?></td>
          </tr>
...
// show reset search
	...
	echo '<a href="new_customer.php" target=_new>Nouveau fournisseur</a>&nbsp;&nbsp;&nbsp;'; 
	echo '&nbsp;&nbsp;&nbsp;';
...
<?php
// echo $_SESSION('language_id');exit;
//    $search = ' where a.entry_country_id in (  ' . get_countries_from_language (  $_SESSION['languages_id']  ) .  ' ) ';
   $search = ' where 1=1';
...
//      $search = "where c.customers_lastname like '%" . $keywords . "%' or c.customers_firstname like '%" . $keywords . "%' or c.customers_email_address like '%" . $keywords . "%'";
    ...
    $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_firstname, c.customers_email_address, c.customers_group_pricing, a.entry_country_id, a.entry_company, a.entry_tva_intracom, ci.customers_info_date_of_last_logon, ci.customers_info_date_account_created " . $new_fields . " from " . TABLE_CUSTOMERS . " c left join " . TABLE_CUSTOMERS_INFO . " ci on c.customers_id= ci.customers_info_id left join " . TABLE_ADDRESS_BOOK . " a on c.customers_id = a.customers_id and c.customers_default_address_id = a.address_book_id " . $search . " order by $disp_order";
...
?>
                ...
                <td class="dataTableContent"><?php echo $customers->fields['entry_tva_intracom']; ?></td>

...
if (  ( $code=='USD' ) )
{
  ...
  $dml = "ALTER TABLE `orders_products` CHANGE `usd_euro_rate` `usd_euro_rate` DECIMAL(15,4) NOT NULL DEFAULT '". $value ."'";  
  $db->Execute($dml);
  
  $dml = " update bo_po.orders_products 
			set usd_euro_rate = ". $value ."
			where usd_euro_rate > 1
			and  orders_id in (select orders_id from bo_po.orders where database_code = 'po' ) ";

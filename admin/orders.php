...
// create search filter
  $search = 'and o.customers_id in (  select a.customers_id from address_book a where a.entry_country_id in (  ' . get_countries_from_language (  $_SESSION['languages_id']  ) .  ' ) )';
...
//      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
    ...
    default:
      if (isset($oInfo) && is_object($oInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . zen_href_link(FILENAME_ORDERS_INVOICE, 'oID=' . $oInfo->orders_id) . '" TARGET="_blank">' . zen_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a> <a href="' .  zen_href_link('rtf/ezl_rtf_bl.php', 'numCommandes=' . $oInfo->orders_id.'&lg_code='.$_SESSION['languages_id']) . '" TARGET="_blank">' . zen_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a>');

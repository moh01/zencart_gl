...
//include "../../php/phpExcel/Tests/05featuredemo.inc.php";
 ...
 function get_countries_from_language ( $p_language_id )
 {
    ...
    {
        return 'not in (21,73,76,195,196,81,105,182,46,45,222,124)';    
    }
 }  
...
//$objPHPExcel = PHPExcel_IOFactory::load("includes/models/reporting3_complet.xls"); --> Marche mais pas de totaux
// $objPHPExcel = PHPExcel_IOFactory::load("includes/models/reporting3.xlsx");
$objPHPExcel = PHPExcel_IOFactory::load("includes/models/reporting4.xls");

//$objPHPExcel = PHPExcel_IOFactory::load("includes/models/reporting3_0.xls"); --> bug
...
//echo '?'.$ts_id.'?';
  ...
  $sql = "select distinct treatment_date 
		  from bo_po.orders 
		  where treatment_date <= '".$_GET['treatment_date']. "'
		  and   treatment_date  >= date_format('".$_GET['treatment_date']. "' ,'%Y-%m-01')
		  order by 1 desc ";
   ...
   while (! $rsMain->EOF )
   {
   		$treatment_date=$rsMain->fields['treatment_date'];

		  $totalMarge = 0;
		  $totalQte = 0;
		  $MargeMoyenne = 0;
		
		  $ligneBase = $ligneMax + 6;
		$treatment_date_from = $treatment_date;
		$treatment_date_to = $treatment_date;
		$cel_ref = getExcelRef(1).$ligneBase;
		...
		*/					
		...
		/';
		  ...
		  for ($k=1;$k<=$cnt_cols;$k++)
		  {
			 ...
			 $sql = "SELECT sum(products_quantity*margin) marge,
					  sum(products_quantity) qty
				FROM orders o,
				   orders_products
				WHERE o.orders_id >0
		...
		*/					
			 ...
			 {   
				...
				if ( $col_db[$k] == "eu" )
				{
				  $totalMarge += $rs->fields["marge"];
				  $totalQte += $rs->fields["qty"];
				}		
				$rs->MoveNext();
				$numLigne++;		
			 }
		 ...
		 $objPHPExcel->getActiveSheet()->setCellValue($cel_ref, 'Total');

		  
		  set_cell_value(43,round($totalMarge) );
		  set_cell_value(44,round($totalQte) );
		  set_cell_value(45,round($totalMarge/$totalQte));
  ...
  if (true)
  {
		...
		$treatment_date_to = $treatment_date;
		$cel_ref = getExcelRef(1).$ligneBase;
		$objPHPExcel->getActiveSheet()->setCellValue($cel_ref, "Commandes annulees");
		...
		'SHF', 'CODF', 'ECOF', 'ESCC','ESCF', 'FRSH', 'FRS', 'SP400','DUSTGO','INSUR'
			  ...
			  AND treatment_date <= '".$_GET['treatment_date']. "'
			  and   treatment_date  >= date_format('".$_GET['treatment_date']. "' ,'%Y-%m-01')
			  and  orders_status = 6
				and o.database_code = '".$col_db[$k]."'
				". $add_where . "

...
echo $sql.'<br>';
		...
		while ( ! $transaction->EOF )
		{
			...
			$sql 	= "select  * from orders where orders_status<>7 and  order_total*100=".$transaction->fields['amount'] . " and customers_id = " . $transaction->fields['customers_id'] ;
echo $sql.'<br>';
			...
			{
			    $dml = "update orders set payment_info = ".$transaction->fields['authorisation_id']." ,  orders_date_finished=now(), payment_amount = order_total  where orders_id = " . $ord->fields['orders_id'] ;
				echo $dml.'<br>';
			    $db->Execute($dml);

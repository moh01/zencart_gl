  ...
  class order {
    ...
    function query($order_id) {
      ...
      $order = $db->Execute("select cc_cvv, customers_name, customers_company, customers_street_address,
                                    ...
                                    order_total, order_tax, ip_address, customers_id, languages_id, 
									database_code, entry_tva_intracom, products_tax, ref_info,
									payment_conditions_desc, payment_conditions_code, 
									payment_info,orders_date_finished,payment_amount,
									payment_info2,orders_date_finished2,payment_amount2
                             from " . TABLE_ORDERS . "
      ...
      $this->info = array('currency' => $order->fields['currency'],
                          ...
                          'languages_id' => $order->fields['languages_id'],
                          'database_code' => $order->fields['database_code'],
                          'ip_address' => $order->fields['ip_address'],
                          'payment_conditions_code' => $order->fields['payment_conditions_code'],
                          'payment_conditions_desc' => $order->fields['payment_conditions_desc'],
                          'orders_date_finished' => $order->fields['orders_date_finished'],						  
                          'products_tax' => $order->fields['products_tax'],
                          'payment_info' => $order->fields['payment_info'],						  						  
                          'payment_amount' => $order->fields['payment_amount'],						  						  						  
                          'ref_info' => $order->fields['ref_info'],						  						  
                          'payment_info2' => $order->fields['payment_info2'],						  						  
                          'payment_amount2' => $order->fields['payment_amount2'],						  						  						  
                          'orders_date_finished2' => $order->fields['orders_date_finished2'],						  						  						  
                          );
//echo "lll".$order->fields['payment_info2']."lll";

      $this->customer = array('name' => $order->fields['customers_name'],
							  'id' => $order->fields['customers_id'],
                              'company' => $order->fields['customers_company'],
                              'entry_tva_intracom' => $order->fields['entry_tva_intracom'],							  
      ...
      $orders_products = $db->Execute("select orders_products_id, products_name, products_model,
                                              ...
                                              product_is_free, products_id, sort_order, reliquat
                                       from " . TABLE_ORDERS_PRODUCTS . "
                                       where orders_id = '" . (int)$order_id . "'
									   order by sort_order,orders_products_id");
...
// convert quantity to proper decimals - account history
        ...
        $this->products[$index] = array('qty' => $new_qty,
                                        ...
                                        'sort_order' => $orders_products->fields['sort_order'],										
                                        'reliquat' => $orders_products->fields['reliquat'],																				

...
function el_format_bd($raw_datetime) {
    if ( ($raw_datetime == '0001-01-01 00:00:00') || ($raw_datetime == '') ) return false;

    $day = substr($raw_datetime, 0, 2);
    $month = substr($raw_datetime, 3, 2);
    $year = substr($raw_datetime, 6, 4);
...
// only show $current_categories_id
  ...
  function zen_address_format($address_format_id, $address, $html, $boln, $eoln) {
	...
	if ( strlen($street) > 27 )
	{
	   $street2pos = strpos($street," ",25);
	   if ( $street2pos )
	   {
    	   $street1 = substr( $street, 0, $street2pos );
		   $street2 = substr( $street, $street2pos, 500 );

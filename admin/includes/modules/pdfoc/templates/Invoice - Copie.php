...
*/
	...
	$PDFOC_PAYMENT_METHOD[4] =  'Versand:';
	...
	$PDFOC_ENTRY_SOLD_TO[3] =  'Direcicón de Pago:';
	...
	$PDFOC_ENTRY_SUBTOTAL[5] =  'Imponibile:';
...
//	echo '.'.$order->info['orders_status'].'.';exit;
	...
	$PDFOC_TEXT_CUSTOMER_NUMBER[4] = 'Kunde Nummer:';
	...
	$PDFOC_TEXT_ORDER_DATE[3] = 'Fecha de pedidos:';
	$PDFOC_TEXT_ORDER_DATE[4] = 'Vom:';
  ...
  {
     $store_address = "SAS Easylamps - Lampevideoprojecteur  
33 rue de la Révolution
93100 Montreuil - France
SIRET : 489 702 514 00030   INTRACOM : FR 68 489 702 514";
  }
...
{
    $check_tax  = $db->Execute("select order_tax from orders where  orders_id = '" . $orders->fields['orders_id'] . "'");
	
	if ( $check_tax->fields['order_tax'] == 0 )
	{
...
{
    $store_address = "SAS Net-Certificate 
1-3 rue Parmentier
93100 Montreuil - France
SIRET: 511 252 991 00019 	TVA INTRACOM: FR 29 511 252 991";
}
  ...
  $pdf->line(PDFOC_LEFT_MARGIN,$y,PDFOC_LINE_LENGTH,$y);
    ...
    //
    $verify_debit = $db->Execute("select * from " . TABLE_ORDERS_INVOICES . " where  order_total<>0 AND orders_id = '" . $orders->fields['orders_id'] . "'");
	...
	{  
      ...
      if ( $verify_debit->fields['invoice_type'] == "DB" )
	  {
	  ...
	  else if ( $verify_debit->fields['invoice_type'] == "CR" )
	  {
...
/*	
  ...
  {	
	  ...
	  for ($i = $nexttotal, $n = sizeof($order->totals); $i < $n; $i++) {
	    ...
	    $pdf->ezText( $bold_begin. pdfoc_html_cleanup($order->totals[$i]['title']) . $bold_end ,PDFOC_TABLE_HEADER_FONT_SIZE,array('justification'=> 'right','aright'=>$x));
	    $x += PDFOC_PRICING_COLUMN_SIZES;
	    $pdf->ezSetY($pos+PDFOC_PRODUCT_TOTALS_LEADING);
	    $pdf->ezText(pdfoc_html_cleanup( $bold_begin. $order->totals[$i]['text'] . $bold_end),PDFOC_TABLE_HEADER_FONT_SIZE,array('justification'=> 'right','aright'=>$x));

...
function get_ups_track($p_orders_id)
{
   ...
   $tracking = str_replace ( "<TrackingNumber>", "", $tracking );
...
//echo  $carrier. '|' .$order_id. '|' .$track. '<br>';
	 ...
	 foreach ($bds as $dtb) 
	 {
		$sql = "select 1 value from ".$ext_db_database[$dtb].".orders where orders_id = ".$order_id. "   and date_purchased >  DATE_SUB(CURDATE(),INTERVAL 140 DAY) ";
...
//echo $bls . '<br><br><br><br>';
				...
				else if($languages_id == '7')
				{ // pl
				  $title = "Wysylanie zamowienia nr: ".$commande_id."";
				  $corps = "
					<p>Szanowny Kliencie,</p>
					<p> Mamy przyjemnosc poinformowac, iz Panstwa paczka zostala wyslana.</p>
					<p> Paczka, wysylana serwisem  ".$carrier." o numerze referencyjnym: ".$track." na nastepujacy adres:  </p><p>
					".$street.", ".$postcode.", 
					".$city.", ".$country.". </p>
					<p>Sledzenie trasy wysylki mozliwe jest za posrednictwem ponizszego linku: </br> 
					<a href=".$check_url.">".$check_url."</a></p>											
					<p> W przypadku pytan odnosnie dostawy, prosimy o kontakt z ".$carrier." </p>		
					" . $bls . "					
					<p> Dziekujemy za dokonanie transakcji.</p>
					";	
				}
...
//				$mail->set_receiver_email_address('frederic.varon@cyber-sphinx.com');
				...
				{
					if( ($languages_id == '2') && ($dtb=="fr")) {
//						mailup_mail("", $customers_email_address, 55, array("commande_id" => $commande_id, "carrier" => $carrier, "track" => $track, "street" => $street, "city" => $city, "postcode" => $postcode, "country" => $country, "check_url" => $check_url), "", "");
						mailup_mail("", $customers_email_address, 55, array("commande_id" => $commande_id, "carrier" => $carrier, "track" => $track, "street" => $street, "city" => $city, "postcode" => $postcode, "country" => $country, "check_url" => $check_url), "", "");
					} else {
						$mail->send_email();
					}
...
echo 'email sent ...........<br>';
					...
					$mail->set_receiver_email_address('fvaron@tsr-informatique.fr');
					$mail->send_email();					
				}
/*				
				echo '</br> Bonsoir';
...
//	echo $day;exit;
	...
	$sql = 'select orders_id from bo_po.orders 
	        where date_purchased >  DATE_SUB(CURDATE(),INTERVAL 5 DAY)  
			and carrier = "UPS"';
...
//            $url = "http://www.station-chargeur.com/appli/v01/com/pop_extract.php?idExtraction=2944&sPasse=&ModeURL=1";
            $url = "http://www.station-chargeur.com/appli/vdhl/com/pop_extract.php?idExtraction=2944&sPasse=&ModeURL=1";
			...
			$error = $http->ReadReplyBody($body, 128000);
			

			$lignes=explode("
",$body);
//echo 'DDDDDDDDDDDD'.count($lignes);exit;			
			$tracking_seq = 2;
			$order_seq = 3;
			$status_seq = 4;			 			 

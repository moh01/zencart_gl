<?php
# FV on oublie le panneau d'acceuil
//echo 'a';exit;

require('includes/application_top.php');

zen_redirect('el_gestion.php');
...
?>
 ...
 <div class="reportBox">
  ...
  // 
  $orders_raclette = $db->Execute("select count(*) as cnt from el_raclette where order_total > 0 ");
  $orders_contents .= '<div class="row"><span class="left"><a href="' . zen_href_link('raclette_handler.php') . '">Raclettes</a>:</span><span class="rigth"> ' . $orders_raclette->fields['cnt']  . '</span>   </div>';
...
<div class="header"><?php echo BOX_ENTRY_NEW_CUSTOMERS; ?> </div>
  ...
  <?php  
     $sql = "select c.customers_id as customers_id, 
	               c.customers_firstname as customers_firstname, 
				   c.customers_lastname as customers_lastname, 
				   a.customers_info_date_account_created as customers_info_date_account_created, 
				   a.customers_info_id 
		       from " . TABLE_CUSTOMERS . " c 
			   left join " . TABLE_CUSTOMERS_INFO . " a 
			        on c.customers_id = a.customers_info_id 
			   where c.customers_id 
			          in (  select a.customers_id 
					        from    address_book a 
							where a.entry_country_id in (  " . get_countries_from_language (  $_SESSION['languages_id']  ) .  " )
					      )
			   order by a.customers_info_date_account_created DESC limit 5" ;
...
<div class="header"><?php echo BOX_ENTRY_NEW_ORDERS; ?> </div>
  <?php  
    ...
    $sql = "select o.orders_id as orders_id, 

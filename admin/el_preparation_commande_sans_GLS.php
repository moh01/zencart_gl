...
//  gestion des partiels
  ...
  function log_error($msg)
  {
	...
	$dml = "insert bo_po.el_dispatch_holes  ( orders_id,
			dispatch_date,
			hole_comment)
			values ( ".  $_SESSION['orders_id'] . ", now(),'"."(".$to_fetch_initial .")". addslashes($msg) ."' )	"; 
	$db->Execute($dml);		
  }
...
*/				
			  ...
			  if ( (strlen($_SESSION['alternatif'])>0) 
					...
					||  ( $to_fetch=='COLLISSIMO' ) ||  ( $to_fetch=='RETRAIT' )  ) )
			  {
					$sql = " select 1 value
...
//	echo '<hr>'.$sql .'<hr>';						
	...
	(". $to_fetch.",  ". $orders_products_id ." , 'dispatched') ";
			  ...
			  else if (  ( $to_fetch=='UPS' ) ||  ( $to_fetch=='DHL' ) ||  ( $to_fetch=='COLLISSIMO' ) ||  ( $to_fetch=='RETRAIT' ) )
			  {
...
//echo 'check_production_bl '. $check_production_bl .'  <br>';
				...
				if ( (  ($database_code=="eu") ||  ($check_production_bl>0) ) && ( strtoupper($payment_module_code) != 'COD' ) )
				{
				$sql = 'select count(1) value
...
//		  echo '<tr><td><textarea rows=10 cols=13 name=cmd_codes>'. $_POST['input_codes'] . '</textarea></td></tr>';
		  echo '<tr>
		            <td>Saisie  <b>CMD [numéro commande]</b> ou <b>[numéro de pièce en stock]</b> ou <b>[TRANSPORTEUR/UPS/DHL/COLLISSIM0] - RETRAIT</b></td>
				</tr>
				<tr>

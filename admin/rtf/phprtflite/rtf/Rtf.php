...
define('PREFIXE_SPECIAL_FONT', 'special font: ');
...
class Rtf {
	...
	*/  	
	 ...
	 * @access public
	 */
	function addFont($font, $special = false) {  
	  	if (!empty($font)) {		  		  	
		  	if (empty($this->fonts[$font])) {			    
				$count = count($this->fonts);
				$fontName = $special ? PREFIXE_SPECIAL_FONT . $font : $font;
				$this->fonts[$fontName] = '\f'.$count;
			}
		}		
	}
	 ...
	 * @access public
	 */
	function getFont($font, $special = false) {	  
		$fontName = $special ? PREFIXE_SPECIAL_FONT . $font : $font;
	...
	function getFontTable() {  	 
	    ...
	    foreach ($this->fonts as $key => $value) {
		  	$part .= '{'.$value;	
			$fontIsSpecial = (0 === strpos($key, PREFIXE_SPECIAL_FONT));	
			$part .= $fontIsSpecial ? '\fnil\fcharset0' : '';	
		  	$part .= ' ';	
			$part .= $fontIsSpecial ? substr($key, strlen(PREFIXE_SPECIAL_FONT)) : $key;	
		  	$part .= ';}';	
		}
		$part .= '}'."\r\n";		    
		return $part;

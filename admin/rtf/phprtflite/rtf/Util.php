...
class Util {	
	 ...
	 * @static	 
	 */	
	function utf8Unicode($str, $currentFontIndex, $specialFontIndex) {
	  	return Util::unicodeToEntitiesPreservingAscii(Util::utf8ToUnicode($str), $currentFontIndex, $specialFontIndex);
	}
...
*/	
	 ...
	 * @static
	 */
	function unicodeToEntitiesPreservingAscii($unicode, $currentFontIndex, $specialFontIndex) {
	    $entities = '';    
	    foreach( $unicode as $value ) {    	
			if ($value != 65279) {
		        $entities .= ( $value > 127 ) ? $specialFontIndex . "\'" . dechex($value) . $currentFontIndex : chr( $value );        
		    }
	    }     
	    return $entities;    
	}

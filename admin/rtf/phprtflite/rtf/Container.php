...
class Container {
	...
	function writeText($text, &$font, &$parFormat, $replaceTags = true) {				
		...
		if (is_a($font, 'Font')) {
			$currentFontName = $font->font;
			$currentFontIndex = $font->getContent($this->rtf);
		} else {
			$currentFontName = 'Arial';
			$currentFontIndex = $this->rtf->getFont($currentFontName, false);
		}
		// Rechreche de la référence de font speciale correspondant à la font courante (quitte à la créer)
		$this->rtf->addFont($currentFontName, true);	
		$specialFontIndex = $this->rtf->getFont($currentFontName, true);
		$text = Util::utf8Unicode($text, $currentFontIndex, $specialFontIndex);		
		//content formating

...
//		  echo $to_fetch;exit;
  ...
  function stock_output ( $chaine_scannee, $i, $silent=0 )
  {
// echo $chaine_scannee;exit; 
	...
	if (strlen($chaine_scannee)>3)
	{
	  ...
	  $to_fetch = str_replace(" SI" ,"" ,$to_fetch);
		...
		$sql = "select exit_date, lamp_code, ctr_code, consignment_stock  from el_stock_items where id= ".$to_fetch;
		$rs=$db->Execute($sql);
		if ($rs->EOF)
		{
		...
		else
		{
			...
			if ($exit_date=="0000-00-00")
			{
				...
				$dml = "update el_stock_items set exit_date = now()  where id = ". $to_fetch;				
				$db->Execute($dml);
				
				$dml = "update el_stock set qty=qty-1  where ctr_code='". $ctr_code."' and lamp_code='". $lamp_code."'";
				$db->Execute($dml);
				...
				{
					$dml = "update el_external_stock set qty=qty-1  where ctr_code='". $ctr_code."' and lamp_code='". $lamp_code."'";										
...
//9088				
					...
					$rs2=$db->Execute($sql);
					$new_external_qty = $rs2->fields["qty"];
					
					$dsp_cs_stock  = '<font color=red> CS STOCK: ' . $new_external_qty.'</font>';
					
				}				
								
				echo '<font color=green>Sortie de stock OK '. $i . ', identifiant : '. $to_fetch .' LAMPE: '. $lamp_code . '  STOCK: ' . $new_qty  . $dsp_cs_stock .'</font><br>' ; 							
				
			}
			else
			{
				echo '<font color=red>Ligne de stock non trouvée pour '. $ctr_code."' and lamp_code='". $lamp_code . '</font><br>' ; 						   
			}
		}
    }
    else
...
<Receiver>
	...
	<ContactPerson>'.removeaccents_common($rs->fields['delivery_name']).'</ContactPerson>
	...
	<EmailAddress1>'.removeaccents_common($rs->fields['customers_email_address']).'</EmailAddress1>
</Receiver>
<Shipper>
